...
class Scenario(BaseScenario):
    def make_world(self, args, config):
        ...
        assert world.human_num > world.human_num_range or world.human_num == 0
    ...
    def convert_to_grid_cell(self, a):
        ...
        a0 = a/(2*self.size_map)*2.*np.pi
        a1 = a/(2*self.size_map)*2.*np.pi*math.sqrt(2)
        a2 = a/(2*self.size_map)*2.*np.pi*2
        a3 = a/(2*self.size_map)*2.*np.pi*2*math.sqrt(2)

        return np.array([(math.cos(a0[0])+math.cos(a0[0]/2+math.sqrt(3)/2*a0[1])+math.cos(a0[0]/2-math.sqrt(3)/2*a0[1])),
                         (math.cos(a1[0])+math.cos(a1[0]/2+math.sqrt(3)/2*a1[1])+math.cos(a1[0]/2-math.sqrt(3)/2*a1[1])),
                         (math.cos(a2[0])+math.cos(a2[0]/2+math.sqrt(3)/2*a2[1])+math.cos(a2[0]/2-math.sqrt(3)/2*a2[1])),
                         (math.cos(a3[0])+math.cos(a3[0]/2+math.sqrt(3)/2*a3[1])+math.cos(a3[0]/2-math.sqrt(3)/2*a3[1])),]
                        )
    ...
    def observation(self, id, world):
        ...
        spatial_edges = np.ones((a, int(2*(world.predict_steps+1)))) * np.inf
        pred_pos = np.transpose(predicted_states[:, :, :2], (1, 0, 2))
        spatial_edges[new_mask] = (pred_pos - np.array([robot.px, robot.py])).reshape((a, -1))[new_mask]
        ...
        if self.args.archi_name == "DSRNNpp":
            ...
            ob = np.empty((2*a+1, max(a, 15)))
            ob[:a, 0] = visible_entities
            ob[:a, 1:13] = sorted_spatial_edges
            one_hot = np.ones((a, 2))
            one_hot[:, 1] = 0
        ...
        t = np.isinf(spatial_edges).any()
        if t:
            print(np.isinf(spatial_edges).any())

        size_info_entity = dim_pos + 1 + self.args.label_entity if self.args.label_entity > 1 else dim_pos + 1

        ob = np.empty((2*a+1, max(a, size_info_entity)))
        ob[:a, 0] = visible_entities
        ob[:a, 1:(dim_pos+1)] = spatial_edges
        if self.args.label_entity > 1 :
            one_hot = np.zeros((a, self.args.label_entity))
            for ind_h, h in enumerate(world.humans):
                if h.policy.__class__.__name__ == 'ORCA':
                    one_hot[ind_h] = [0,0,1]
                elif h.policy.__class__.__name__ == 'SOCIAL_FORCE':
                    one_hot[ind_h] = [0,1,0]
                else:
                    raise NotImplementedError
            one_hot[-world.num_agents:] = [1,0,0]
            ob[:a, (dim_pos+1):(dim_pos + 1 + self.args.label_entity)] = one_hot
        ob[a:(2*a), :a] = visi
        ob[2*a, :len(first_line_obs)] = first_line_obs
