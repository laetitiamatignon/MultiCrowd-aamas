...
class Scenario(BaseScenario):
    ...
    def generate_humans(self, world, human_num):
        ...
        :return: None
        """
        self.line_humans_pos = []
        for l in range(self.num_line_humans):
            pos_line = np.random.uniform(-1, 1, 4)
            self.line_humans_pos.append([pos_line[0], pos_line[1], pos_line[2], pos_line[3]])
            for i in range(int(human_num/self.num_line_humans)):
    ...
    def generate_human(self, world):
        ...
        while True:
            ...
            px = np.random.uniform(self.line_humans_pos[-1][0]*world.size_map,
                                    self.line_humans_pos[-1][1]*world.size_map,
                                   1)
            py = np.random.uniform(self.line_humans_pos[-1][2]*world.size_map,
                                    self.line_humans_pos[-1][3]*world.size_map,
                                   1)
            px = px + px_noise
            py = py + py_noise
            gx = px
    ...
    def generate_agent(self, world):
        ...
        if agent.kinematics == 'unicycle':
            ...
            while True:
                px_noise = (np.random.uniform() - 0.5) * agent.v_pref
                py_noise = (np.random.uniform() - 0.5) * agent.v_pref
                px = np.random.uniform(-world.size_map, world.size_map, 1)
                py = np.random.uniform(-world.size_map, world.size_map, 1)
                gx = px + px_noise
                gy = -py + py_noise
                collide = False
    ...
    def observation(self, id, world):
        ...
        # ESTEST
        # prendre les distances entre tous
        position = world.cur_states[:,:2]
        dist = position[:,np.newaxis] - position
        # prendre la vélocité de tous
        vel = world.future_traj[0, :, 2:4]
        # prendre la visibilité entre tous
        visi = world.visibility
        for i in range(visi.shape[0]):
            visi[i,i] = False
        # print(visi.shape)
        # print(dist.shape)
        # print(vel.shape)
        # print(num_visible_entities)
        ob = np.empty((3*num_visible_entities + 3,num_visible_entities, 4))
        ob[:num_visible_entities, :, 0] = visi
        ob[num_visible_entities:2*num_visible_entities, :2, 0] = vel
        ob[2*num_visible_entities:3*num_visible_entities, :, :2] = dist
        ob[2*num_visible_entities:3*num_visible_entities, :, 2:4] = dist
        ob[3*num_visible_entities, 0, :] = first_line_obs[:4]
        ob[3*num_visible_entities + 1, 0, :] = first_line_obs[4:8]
        ob[3*num_visible_entities + 2, 0, :] = first_line_obs[-1]
