...
class Scenario(BaseScenario):
    ...
    def generate_humans(self, world, human_num):
        ...
        """
        num_wave = 3
        num_human_per_wave = int(human_num/num_wave)

        for i in range(num_wave):
            # if i == num_wave and num_human_per_wave*num_wave != human_num:
            #     num_human_per_wave =+ 1
            self.generate_wave(world, num_human_per_wave, i, num_wave)
        
        if human_num - num_wave*num_human_per_wave != 0:
            self.generate_wave(world, human_num - num_wave*num_human_per_wave, num_wave, num_wave)
    ...
    def generate_wave(self, world, num_human_per_wave, wave, wave_tot):
        angle = np.random.random() * np.pi * 2
        dist_wave = float(wave)/float(wave_tot) + 0.5

        # fourchette:
        #  a pos du robot entre -1 et 1 (ou plus potentiellement)
        #  x  =  Rcost - a * Rsint
        #  y  =  Rsint + a * Rcost

        for h in range(num_human_per_wave):
            ...
            while True:
                ...
                pos_noise = np.random.uniform(-0.5,0.5,1) * v_pref
                px_noise = (np.random.uniform() - 0.5) * v_pref
                py_noise = (np.random.uniform() - 0.5) * v_pref
                px = dist_wave * world.size_map * np.cos(angle) - pos_noise * world.size_map * np.sin(angle) # + px_noise
                py = dist_wave * world.size_map * np.sin(angle) + pos_noise * world.size_map * np.cos(angle) # + py_noise
                gx = -px
                gy = -py
                collide = False
            ...
            human.set(px, py, gx, gx, 0, 0, 0) 
            world.humans.append(human)
