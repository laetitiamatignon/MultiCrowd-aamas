#!/bin/sh
env="CROWD"
scenario="simple_circle_obstacle"
num_agents=1
algo="rmappo"
exp="check"
seed_max=1

echo "env is ${env}"
for seed in `seq ${seed_max}`
do
    CUDA_VISIBLE_DEVICES=0 python3 ./scripts/render/render_crowd.py --share_policy --env_name ${env} --algorithm_name ${algo} \
    --experiment_name ${exp} --scenario_name ${scenario} --num_agents ${num_agents} --seed 1000 --visualize True --visualize_traj \
    --episode_length 150 --n_training_threads 1 --n_rollout_threads 1 --data_chunk_length 2 --use_render --label_entity 0 \
    --archi_name "DSRNNoff" --model_dir "./scripts/results/NEW/DSRNN_original/models/41200.pt" --render_episodes 1000 --GHR_edge_selector_num_head 4
done

