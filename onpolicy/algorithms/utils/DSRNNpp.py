...
from onpolicy.algorithms.utils.srnn_model import *
...
class SpatialEdgeSelfAttn(nn.Module):
    ...
    def create_attn_mask(self, each_seq_len, seq_len, nenv, max_human_num):
        ...
        mask[torch.arange(seq_len*nenv), (each_seq_len - 1).long()] = 1.
...
class EdgeAttention_M(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Store required sizes
        self.human_human_edge_rnn_size = args.DSRNN_human_human_edge_rnn_size
        self.human_node_rnn_size = args.DSRNN_human_node_rnn_size
        self.attention_size = args.DSRNN_attention_size
...
class EndRNN(RNNBase):
    ...
    def __init__(self, args):
        ...
        # Store required sizes
        self.rnn_size = args.DSRNN_human_node_rnn_size
        self.output_size = args.DSRNN_human_node_output_size
        self.embedding_size = args.DSRNN_human_node_embedding_size
        self.input_size = args.DSRNN_human_node_input_size
        self.edge_rnn_size = args.DSRNN_human_human_edge_rnn_size
...
class selfAttn_merge_SRNN(nn.Module):
    ...
    def __init__(self, obs_space, args, device=torch.device("cpu"), infer=False):
        ...
        self.entities_num = int((obs_space.shape[-2]-1)/2)
        self.num_agents = args.num_agents
        self.human_num = self.entities_num - self.num_agents
        ...
        self.input_entities_size = self.dim_pos + 2 if args.label_entity else self.dim_pos
        ...
        if self.args.use_self_attn:
            self.spatial_attn = SpatialEdgeSelfAttn(args, self.input_entities_size)
    ...
    def forward(self, inputs, rnn_hxs, masks, tau, infer=False):
        ...
        detected_human_num = inputs[:,-1,9].squeeze(-1).cpu().int()
        ...
        spatial_edges = reshapeT(inputs[:, :self.human_num, 1:(self.input_entities_size+1)], seq_length, nenv)
        ...
        if self.args.cuda:
            all_hidden_states_edge_RNNs = Variable(
                torch.zeros(1, nenv, 1+self.human_num, hidden_states_node_RNNs.size()[-1]).cuda())
        else:
            all_hidden_states_edge_RNNs = Variable(
                torch.zeros(1, nenv, 1+self.human_num, hidden_states_node_RNNs.size()[-1]).cpu())
