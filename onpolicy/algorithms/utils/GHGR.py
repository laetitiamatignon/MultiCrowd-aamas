...
class GHGR(nn.Module):
    ...
    def __init__(self, obs_space, args, device=torch.device("cpu"), infer=False):
        ...
        self.num_entities = obs_space.shape[-1] - 1
        ...
        self.LC_human = LC(self.input_entities_size, args)
        self.LC_agent = LC(self.input_entities_size, args)

        self.lin_concat = init_(nn.Linear(args.mha_emb_size*2, args.mha_emb_size))
    ...
    def forward(self, inputs, rnn_hxs, masks, tau=1., infer=False):
        ...
        spatial_edges = inputs[:, :num_visible_entities, 1:(self.input_entities_size + 1)]
        ...
        input_human = torch.cat((spatial_edges[:, :-self.num_agents], spatial_edges[torch.arange(id_robot.size(0)), id_robot].unsqueeze(1)), 1)
        visibility_human = torch.cat((visibility[:, :-self.num_agents], visibility[torch.arange(id_robot.size(0)), id_robot].unsqueeze(1)), 1)[:,:,:(self.num_entities - self.num_agents + 1)]
        visibility_human[:, :, -1] = 0
        id_robot_human = torch.ones_like(id_robot)

        input_agent = spatial_edges[:, -self.num_agents:]
        visibility_agent = visibility[:, -self.num_agents:, -self.num_agents:]
        id_robot_agent = id_robot - (self.num_entities - self.num_agents)
        ...
        hidden_attn_human = self.LC_human(input_human, visibility_human, id_robot_human, tau)
        hidden_attn_agent = self.LC_agent(input_agent, visibility_agent, id_robot_agent, tau)
        hidden_attn = torch.cat((hidden_attn_human, hidden_attn_agent), -1)

        hidden_attn_weighted = hidden_attn.view(seq_length, nenv, -1).unsqueeze(-2)
        hidden_attn_weighted = self.lin_concat(hidden_attn_weighted)
