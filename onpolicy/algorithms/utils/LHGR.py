...
class LHGR(nn.Module):
    ...
    def __init__(self, obs_space, args, device=torch.device("cpu"), infer=False):
        ...
        input_agent_size = args.mha_emb_size
        ...
        self.LC_human = LC(self.input_entities_size, args, 'LHGR')
        self.LC_agent = LC(input_agent_size, args, 'LHGR')
    ...
    def forward(self, inputs, rnn_hxs, masks, tau=1., infer=False):
        ...
        visibility_human = visibility.detach().clone()
        visibility_human[:, :, -self.num_agents:] = 0

        robot_states = torch.cat((temporal_edges, robot_node), dim=-1)
        robot_states = self.robot_linear(robot_states)

        hidden_attn_human = self.LC_human(spatial_edges.reshape(nenv*seq_length, num_visible_entities, -1), visibility_human, None, tau)
        hidden_attn_human = hidden_attn_human[:, -self.num_agents:]

        hidden_attn_agent = self.LC_agent(hidden_attn_human, visibility[:, -self.num_agents:, -self.num_agents:], id_robot, tau)
        hidden_attn_weighted = hidden_attn_agent.view(seq_length, nenv, -1).unsqueeze(-2)
