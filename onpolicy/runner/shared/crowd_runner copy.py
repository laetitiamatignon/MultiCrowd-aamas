    ...
    def render(self):
        ...
        for episode in range(self.all_args.render_episodes):
            ...
            for step in range(self.episode_length):
                ...
                path_len = path_len + np.linalg.norm(obs[0, 0, 0, :2] - last_pos)
                last_pos = obs[0, 0, 0, :2]
        ...
        collision_rate = collision / (self.all_args.render_episodes * self.num_agents)
        ...
        print(
            ...
            'average minimal distance during intrusions: {:.2f}\n average minimum distance to goal: {:.2f}\n average end distance to goal: {:.2f}\n'.
                format(success_rate, collision_rate, timeout_rate, avg_nav_time, np.mean(all_path_len),
                    np.mean(too_close_ratios), np.mean(dist_intrusion), np.mean(min_dist_goal_tot), np.mean(end_dist_goal_tot)))
